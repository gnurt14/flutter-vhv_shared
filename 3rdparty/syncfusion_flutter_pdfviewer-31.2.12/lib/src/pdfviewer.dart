...
class SfPdfViewerState extends State<SfPdfViewer> with WidgetsBindingObserver {
  ...
  /// Password used to open the encrypted PDF document.
  String? _password;
  ...
  void initState() {
    ...
    _transformationController =
        TransformationControllerExt()..addListener(_updateScrollOffset);
  ...
  void didUpdateWidget(SfPdfViewer oldWidget) {
    ...
    _compareDocument(oldWidget, context);
  ...
  Future<void> _compareDocument(
    SfPdfViewer oldWidget,
    BuildContext context,
  ) async {
    Uint8List? oldBytes;
    Uint8List? newBytes;
    if (widget._source != oldWidget._source ||
        widget.password != null && widget.password != oldWidget.password) {
      try {
        oldBytes = _originalSourceBytes;
        newBytes = await widget._source.getBytes(context);
        if (!listEquals(oldBytes, newBytes) ||
            (widget.password != null &&
                widget.password != oldWidget.password)) {
          _password = null;
          _originalSourceBytes = newBytes;
          _pdfViewerController.clearSelection();
          // PDF document gets loaded only when the user changes
          // the input source of PDF document.
          await _loadPdfDocument(true, false, newBytes);
        }
      } catch (e) {
        ...
        if (errorMessage.contains('Failed to fetch')) {
          widget.onDocumentLoadFailed?.call(
            PdfDocumentLoadFailedDetails(
              'Error',
              'Unable to load the document because the URL is invalid or the device is disconnected.',
            ),
          );
  ...
  void dispose() {
    ...
    _document?.dispose();
    _document = null;
    _pdfPagesKey[_pdfViewerController.pageNumber]?.currentState?.canvasRenderBox
        ?.disposeSelection();
      ...
      if (field is PdfTextBoxField) {
        if (field.items != null && field.items!.count > 0) {
          ...
          for (final PdfTextFormField textFormField in groupedTextFormFields) {
            ...
            final List<PdfTextFormField> groupedItems =
                groupedTextFormFields
                    .where(
        ...
        decoration: InputDecoration(
          ...
          errorBorder: OutlineInputBorder(
            borderSide: BorderSide(
              width: isMaterial3 ? 2 : 1,
              color:
                  _pdfViewerThemeData!.passwordDialogStyle?.errorBorderColor ??
                    ...
                    children: <Widget>[
                      ...
                      Form(
                        ...
                        child: TextFormField(
                          ...
                          focusNode: _focusNode,
                          onFieldSubmitted: (String value) {
                            _handlePasswordValidation(context);
                          },
                          onChanged:
                              isMaterial3
                                  ? (String value) {
  ...
  void _handlePasswordValidation(BuildContext context) {
              ...
              builder: (BuildContext context, BoxConstraints constraints) {
                ...
                if (_deviceOrientation == Orientation.landscape) {
                  _viewportHeightInLandscape ??=
                      MediaQuery.of(context).size.height - _otherContextHeight!;
                }
                ...
                final List<Widget> children = List<
                  Widget
                >.generate(_pdfViewerController.pageCount, (int index) {
                  if (index == 0) {
                    totalHeight = 0;
                  }
  ...
  void _checkVisiblePages() {
    ...
    if (widget.pageLayoutMode == PdfPageLayoutMode.single) {
      ...
      for (
        int pageNumber = _pdfViewerController.pageNumber - 1;
        pageNumber > 0;
        pageNumber--
      ) {
        final Rect pageRect = Rect.fromLTWH(
  ...
  Size _getChildSize(Size viewportDimension) {
    ...
    if (_pdfScrollableStateKey.currentState != null) {
      widthFactor =
          _pdfScrollableStateKey.currentState!.paddingWidthScale == 0
              ? _pdfViewerController.zoomLevel
    ...
    if (_pdfPages[_pdfViewerController.pageCount] != null) {
      ...
      childWidth =
          viewportDimension.width > totalImageWidth
              ? viewportDimension.width /
                  widthFactor.clamp(1, widget.maxZoomLevel)
      ...
      childHeight =
          viewportDimension.height > totalImageHeight
              ? viewportDimension.height /
  ...
  void _handlePointerDown(PointerDownEvent event) {
    ...
    if (!isTextFormFieldFocused) {
      _pdfPagesKey[_pdfViewerController.pageNumber]?.currentState?.focusNode
          .requestFocus();
    }
  }
  ...
  void _handlePointerMove(PointerMoveEvent event) {
    ...
    if (event.kind == PointerDeviceKind.touch) {
      ...
      const double kStylusSlop = kTouchSlop / 3;
      _canInvokeOnTap &=
          draggedDistance.dx.abs() < kStylusSlop &&
          draggedDistance.dy.abs() < kStylusSlop;
    } else {
      _canInvokeOnTap &=
          draggedDistance.dx.abs() < kPrecisePointerHitSlop &&
    ...
    if (kIsWeb || !Platform.environment.containsKey('FLUTTER_TEST')) {
      ...
      _tileTimer ??= Timer(Durations.medium4, () async {
        ...
        if (_pageLayoutMode == PdfPageLayoutMode.continuous) {
          ...
          _pdfPagesKey[_pdfViewerController.pageNumber]?.currentState
              ?.getTileImage(
                _transformationController,
                _viewportSize,
                zoomLevel,
  ...
  void _updateSearchInstance({bool isNext = true}) {
    if (_textCollection != null &&
      ...
      _pdfViewerController
          ._pdfTextSearchResult
          ._currentOccurrenceIndex = _getInstanceInPage(
        _pdfViewerController.pageNumber,
        lookForFirst: isNext,
      );
    }
  ...
  void _updateCurrentPageNumber({double currentOffset = 0}) {
    if (currentOffset > 0) {
      _pdfViewerController._pageNumber =
          _pdfScrollableStateKey.currentState?.getPageNumber() ??
          _pdfViewerController.pageNumber;
    } else {
      if (_textDirection == TextDirection.rtl &&
          _scrollDirection == PdfScrollDirection.horizontal) {
            ...
            onSelected: (String value) {
              if (value != 'Copy') {
                ...
                Clipboard.setData(
                  ClipboardData(
                    text:
                        _pdfPagesKey[_selectedTextPageNumber]
                            ?.currentState
                            ?.canvasRenderBox!
                            .getSelectionDetails()
                            .copiedText ??
  ...
  void _jumpToBookmark(PdfBookmark? bookmark) {
    if (bookmark != null && _document != null) {
      ...
      if (bookmark.namedDestination != null) {
        ...
        heightPercentage =
            bookmark.destination!.page.size.height /
            (isRotatedTo90or270 ? revealedOffset.width : revealedOffset.height);
        widthPercentage =
            bookmark.destination!.page.size.width /
            (isRotatedTo90or270 ? revealedOffset.height : revealedOffset.width);
  ...
  void _getPageSizes() {
    ...
    for (int pageIndex = 0; pageIndex < _document!.pages.count; pageIndex++) {
      ...
      final double width =
          isRotatedTo90or270 ? page.size.height : page.size.width;
      final double height =
          isRotatedTo90or270 ? page.size.width : page.size.height;
