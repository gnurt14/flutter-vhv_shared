...
class _FlexibleBottomSheetState<T> extends State<FlexibleBottomSheet<T>> {
  ...
  void _animateToFocused(ScrollController controller) {
    ...
    _widgetBinding.addPostFrameCallback((_) {
      try{
        ...
        final renderBox = focusContext.findRenderObject() as RenderBox?;
  ...
  Future<void> _checkPos(ScrollController controller)async{
    await Future.delayed(Duration(milliseconds: 300));
    if(!mounted || hasInitial){
      return;
    }
    bool flag = hasInitial;
    int count = 0;
    while(!flag) {
      if(count > 3){
        flag = true;
      ...
      if (controller.hasClients) {
        if(controller.position.pixels > 0){
          flag = true;
          _controller.animateTo(widget.maxHeight, duration: Duration(milliseconds: 300), curve: Curves.linear);
        }else{
          count++;
